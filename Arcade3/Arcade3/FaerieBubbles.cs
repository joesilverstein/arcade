        ...
        private void animationTimer_Tick(object sender, EventArgs e)
        {
            if (tick != 0 && tick % ((60 / (level*2)) * 1000 / animationTimer.Interval) == 0)
            {
                ...
                for (int i = 0; i < nextBubbles.Count; i++)
                    nextBubbles.ToArray()[i].setRectangleY(myCanvas.Height - bubbleSize.Height, bubbleSize);
                numCanvasChanges++;
            ...
            if (launchBubble)
            {
                ...
                for (int i = 0; i < nextBubbles.Count; i++)
                    nextBubbles.ToArray()[i].changeRectangleX(-bubbleSize.Width,bubbleSize);
            ...
            if (bubbleFlying)
            {
                PointF FBpoint = bubbleList[((bubbleList.Count) - 1)].getRectangle().Location;
                ...
                bubbleList[(bubbleList.Count - 1)].setloc(FBpoint, bubbleSize);
            ...
            for (int j = 0; j < nextBubbles.Count; j++)
                nextBubbles.ToArray()[j].DrawBubble(g);
